...
To make the sparse comparison fair at a matched feature budget, we
additionally retrain XGBoost on the same 20 features selected for
the sparse ChebyKAN hero with hyperparameters re-tuned for that
smaller input space (max\_depth 6, 300 trees); this is the
``XGBoost (20 ft)'' row.
...
\begin{table}[H]
\centering
\caption{Held-out outer-test QWK and structural size, grouped by feature budget. Dense KAN rows report the mean $\pm$ standard deviation across three training seeds; the other rows are single retrainings with 95\% bootstrap CIs over n{=}11{,}877 test samples. At matched feature budgets, both KAN flavours clearly beat XGBoost: dense KANs by 0.03--0.04 QWK over full-feature XGBoost, sparse KANs by 0.06--0.09 QWK over 20-feature XGBoost.}
...
XGBoost (20 ft, retuned)   & 0.470 [0.454, 0.487] & 66k leaves  & SHAP (post-hoc) \\
ChebyKAN, sparse (no LN)   & 0.533 [0.519, 0.547] & \textbf{597} & \textbf{closed form + exact Greeks} \\
FourierKAN, sparse (no LN) & 0.562 [0.549, 0.576] & 7{,}158     & per-edge closed form \\
\hline
\end{tabular}
\end{table}
...
At both feature budgets in Table~\ref{tab:interpretability_outer_test},
TabKAN outperforms XGBoost on outer-test QWK: by 0.03--0.04 at full
features and by 0.06--0.09 at the 20-feature budget. The sparse
ChebyKAN row additionally combines three properties no other row
achieves simultaneously: 44$\times$ edge sparsity against the dense
baseline (597 vs.\ 26{,}112), a single closed-form composed
expression in the 20 inputs, and exact analytic Greeks. The accuracy
cost of these three properties, against the best-performing TabKAN
configuration in the paper, is about 0.06 QWK on held-out test.

\section{Discussion and Limitations}
\label{sec:discussion}

Finally, we discuss the implications and limitations of our findings. In particular, we consider:

\begin{itemize}
    \item whether observed performance differences are statistically and practically meaningful,
