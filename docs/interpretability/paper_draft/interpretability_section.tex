...
smaller input space (max\_depth 6, 300 trees, gamma 1); this is the
``XGBoost (20 ft)'' row. The XGBoost implementation used throughout
is XGBRegressor on the ordinal target followed by
...
\begin{table}[H]
\centering
\caption{Held-out outer-test QWK and structural size, grouped by feature budget. Dense KAN rows report the mean $\pm$ standard deviation across three training seeds; all other rows are single retrainings with 95\% bootstrap CIs over n{=}11{,}877 test samples. At full features XGBoost is the strongest baseline (0.642); at 20 features the sparse FourierKAN matches XGBoost within bootstrap noise and the sparse ChebyKAN trails by $\approx 0.03$ QWK in exchange for a single closed-form composed expression and exact analytic Greeks.}
...
XGBoost (20 ft, retuned)   & 0.565 [0.551, 0.579] & 300 trees   & SHAP (post-hoc) \\
ChebyKAN, sparse (no LN)   & 0.533 [0.519, 0.547] & \textbf{597 edges} & \textbf{closed form + exact Greeks} \\
FourierKAN, sparse (no LN) & 0.562 [0.549, 0.576] & 7{,}158 edges & per-edge closed form \\
\hline
\end{tabular}
\end{table}
...
At the 20-feature budget, the sparse FourierKAN matches XGBoost
within bootstrap noise (0.562 vs.\ 0.565) with 6$\times$ fewer active
parameters (7{,}158 edges vs.\ 300 trees). The sparse ChebyKAN row
trails by about 0.03 QWK (0.533 vs.\ 0.565) in exchange for three
properties no other row in Table~\ref{tab:interpretability_outer_test}
achieves simultaneously: 44$\times$ edge compression against its
dense baseline (597 vs.\ 26{,}112), a single closed-form composed
expression in the 20 inputs, and exact analytic Greeks. The
interpretability cost of the sparse ChebyKAN, against the
strongest KAN baseline in the paper, is therefore about 0.06 QWK
on held-out test.
