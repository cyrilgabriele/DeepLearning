...
def test_main_loads_interpret_runtime_config(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    config_payload = {
        "trainer": {
            "experiment_name": "glm-baseline",
    ...
    interpret_config = tmp_path / "interpret.yaml"
    interpret_config.write_text(
        "\n".join(
            [
                f"config: {experiment_config}",
                f"checkpoint: {checkpoint}",
                f"output_root: {tmp_path / 'from-yaml'}",
                "pruning_threshold: 0.03",
                "qwk_tolerance: 0.04",
                "candidate_library: pysr",
                "max_features: 7",
            ]
        )
    )
    ...
    assert captured["config"].trainer.experiment_name == "glm-baseline"
    assert captured["checkpoint_path"] == checkpoint
    assert captured["output_root"] == tmp_path / "from-yaml"
    assert captured["pruning_threshold"] == 0.03
    assert captured["qwk_tolerance"] == 0.04
    assert captured["candidate_library"] == "scipy"
    assert captured["max_features"] == 7
